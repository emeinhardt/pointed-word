...
The first variation — exposed in "PointedWord" — is the simpler one:
...
Note that
 ...
 - There is no ability to represent empty strings anywhere in the interior of a pointed word: the only
   representable pointed word in the "PointedWord" module containing the empty string is the pointed word
   consisting of the empty string itself and no other symbols: @lift "" = GPW Seq.empty@.

This second quality makes it simpler and less expressive than the implementation exposed here in
"PointedWord.Eps",

> newtype GPW b a = GPW { unGPW ∷ NESeq (GPwNode b a) }
> data GPwNode b a = Context b | Point a
> type PW a = GPW (Seq a) a
> type PwNode a = GPwNode (Seq a) a
...
lift' = fmap fuse .: splitAround where
  ...
  fuse (l,p,r) =
    GPW $ unsafeFromSeq
    $  maybe S.singleton (\l' → (S.singleton  (Context l') :|>)) l (Point p)
    >< maybe S.empty            (S.singleton ∘ Context)          r


-- | \"Flatten\" a pointed word into an undifferentiated sequence of values.
lower ∷ ∀ a. PW a → Seq a
lower = F.foldr δ S.empty ∘ unGPW where
  δ ∷ GPwNode (Seq a) a → Seq a → Seq a
  δ (Context as) as' = as >< as'
  δ (Point    a) as' = a :<| as'
...
pointIdxs = (fmap ∘ toLowerIndexed) <*> pointIdxs' where
  ...
  toLowerIndexed w i = i + ala Sum foldMap (fmap (subtract 1 ∘ lengthN) ∘ N.take i $ unGPW w)

-- | Return an ascending 'Seq' of indices of all 'Point's.
pointIdxs' ∷ ∀ b a. GPW b a → Seq Int
pointIdxs' = S.fromList ∘ N.findIndicesL isPoint ∘ unGPW
...
contextIdxs' = S.fromList ∘ N.findIndicesL isContext ∘ unGPW

-- | The rightmost value of a 'PwNode'.
rightmostN ∷ ∀ a. PwNode a → Maybe a
rightmostN (Point           a ) = Just a
rightmostN (Context (_  :|> a)) = Just a
rightmostN (Context S.Empty   ) = Nothing

-- | The leftmost value of a 'PwNode'.
leftmostN ∷ ∀ a. PwNode a → Maybe a
leftmostN (Point           a ) = Just a
leftmostN (Context (a :<|  _)) = Just a
leftmostN (Context S.Empty   ) = Nothing

-- | The length of the contents of a 'PwNode'.
lengthN ∷ ∀ a. PwNode a → Int
lengthN (Point    _) = 1
lengthN (Context as) = S.length as
...
subIdxedN (Context as) = Context $ S.zip [0..(S.length as)] as
...
-- | Zip each value in a 'PW' with its index in the underlying lowered word.
lowerIdxed ∷ ∀ a. PW a → NESeq (PwNode (Int, a))
lowerIdxed = unsafeFromSeq ∘ F.foldl δ S.empty ∘ unGPW ∘ subIdxed where
  δ ∷ Seq (PwNode (Int, a)) → PwNode (Int, a) → Seq (PwNode (Int, a))
  δ S.Empty     p = S.singleton p
  δ s@(_ :|> n) p = s :|> lowerIdxedN ((+1) ∘ fst ∘ fromJust ∘ rightmostN $ n) p

subLookup ∷ ∀ a. PwNode a → Int → Maybe a
subLookup (Point a) i
  | i == 0    = Just a
  | otherwise = Nothing
subLookup (Context as) i = S.lookup i as
...
toNodeIndex p i
  ...
  | otherwise = let
        ...
        indexWithI ∷ Int → NESeq (PwNode (Int, a)) → Int
        indexWithI = fromJust .: N.findIndexL ∘ containsI

        -- given i + a lowerindexed node containing i, calculate the offset
        offset ∷ Int → PwNode (Int, a) → Int
        offset _ (Point    _) = 0
        offset j (Context as) = fromJust $ S.findIndexL ((j ==) ∘ fst) as

        p' = lowerIdxed p
        n  = indexWithI i p'
      in Just (n, offset i ∘ fromJust ∘ N.lookup n $ p')
...
fromNodeIndexL p i = fmap fst ∘ leftmostN =<< N.lookup i (lowerIdxed p)
...
fromNodeIndexR p i = fmap fst ∘ rightmostN =<< N.lookup i (lowerIdxed p)
...
fromNodeIndex p i = do
  n ← N.lookup i $ lowerIdxed p
  l ← fmap fst ∘ leftmostN  $ n
  r ← fmap fst ∘ rightmostN $ n
  return (l, r)
  -- --     uncurry (liftA2 (,))
  -- -- <=< fmap ((fmap fst ∘ leftmostN) &&& (fmap fst ∘ rightmostN))
  -- -- ∘   N.lookup i
  -- -- ∘   lowerIdxed
  -- -- $   p
...
invalidr = out ∘ F.foldr δ (False, S.empty) ∘ unGPW where
  ...
  out (_   , pw      ) = Right ∘ GPW ∘ unsafeFromSeq $ pw
...
maintain = GPW ∘ unsafeFromSeq ∘ F.foldr δ S.empty ∘ unGPW where
...
instance (Semigroup b) ⇒ Semigroup (GPW b a) where
  (GPW (ls :||> (Context l))) <> (GPW ((Context r) :<|| S.Empty     )) = GPW  (ls :||> Context (l <> r))
  (GPW (ls :||> (Context l))) <> (GPW ((Context r) :<|| r'@(_ :<| _))) = GPW ((ls :||> Context (l <> r)) N.>< unsafeFromSeq r')
  (GPW  ls                  ) <> (GPW                   rs           ) = GPW ( ls                        N.>< rs)

instance (Monoid b) ⇒ Monoid (GPW b a) where
  mempty = GPW ∘ N.singleton ∘ Context $ mempty
...
mapWithIdxN f (Context as) = Context $ S.mapWithIndex f as
...
point i p@(GPW w)
  ...
  | otherwise = do
      ...
      let insertl = maybe id              (        N.update            j)      l
      let insertp = maybe (N.update j p') (const $ N.insertAt (offsetp j)  p') l
      let insertr = maybe id              (        N.insertAt (offsetr j))     r
...
unpoint i p@(GPW w)
  ...
  | otherwise = let
      ...
      mergeLR ∷ Int → Seq a → Maybe (PwNode a) → Maybe (PwNode a) → Maybe (Seq (PwNode a) → Seq (PwNode a))
      ...
      mergeL1 ∷ Seq a → PwNode a → Maybe (PwNode a)
      ...
      mergeL ∷ Int → Seq a → Maybe (PwNode a) → Maybe (Seq (PwNode a) → Seq (PwNode a))
